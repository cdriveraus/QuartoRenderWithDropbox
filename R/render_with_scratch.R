...
render_with_temp_file <- function(qmd, outputtype = NULL, ...) {
  ...
  lines <- c(
    ...
    "# ---- identify artefacts to copy back (only typical outputs + cache/freeze) ----",
    "allowed_ext <- c('html','pdf','docx','pptx','epub','odt','rtf','ipynb','tex')",
    ...
    "  if (dir.exists(item)) {",
    "    # match _files dirs",
    "    if (identical(b, paste0(stem, '_files')) ||",
    "        (startsWith(b, paste0(stem, '-')) && endsWith(b, '_files'))) {",
    "      needs_copy[[length(needs_copy)+1]] <- item",
    "    }",
    "    # also bring back cache and freeze dirs if present",
    "    if (identical(b, paste0(stem, '_cache')) || identical(b, '_freeze')) {",
    "      needs_copy[[length(needs_copy)+1]] <- item",
    "    }",
    "  } else {",
    ...
    "        (identical(base_no_ext, stem) || startsWith(base_no_ext, paste0(stem, '-')))) {",
    "      needs_copy[[length(needs_copy)+1]] <- item",
    "    }",
    "  }",
    "}",
